...
function updateTaskStatus(task: Task, status: boolean) {
    task.status = status
    ...
    <div class="flex flex-col gap-4">
        <div class="flex justify-between items-center px-6 md:px-0">
            ...
            <div class="dropdown dropdown-end">
                <div tabindex="0" role="button" class="bg-base-100 m-1 btn">
                    <svg
                        class="w-5 h-5 text-base-content" data-slot="icon" fill="none" stroke-width="1.5"
                ...
                <div tabindex="0" class="z-[1] bg-base-100 shadow p-2 rounded-box dropdown-content menu">
                    ...
                    <div class="flex flex-col gap-4 px-3 py-2 min-w-52">
                        <label class="form-control w-full max-w-xs">
                            ...
                            <input
                                v-model="filters.query" type="text" placeholder="Eg. My shopping list..."
                            ...
                            <div class="flex justify-start gap-3">
                                <div class="form-control">
                                    <label class="flex justify-start gap-3 cursor-pointer label">
                                        <input
                                            v-model="filters.status" type="radio" name="status-radio" :value="true"
                                ...
                                <div class="form-control">
                                    <label class="flex justify-start gap-3 cursor-pointer label">
                                        <input
                                            v-model="filters.status" type="radio" name="status-radio" :value="false"
                        ...
                        <label class="form-control w-full max-w-xs">
                            ...
                            <select v-model="filters.category" class="select-bordered select-sm select">
                                <option :value="null" disabled selected>Pick one</option>
                                <option v-for="val of categories" :key="val.id" :value="val.id">
                                    {{ val.name }}
                                </option>
                            </select>
                        </label>
                        ...
                        <label class="form-control w-full max-w-xs">
                            ...
                            <select v-model="filters.priority" class="select-bordered select-sm select">
                                <option :value="null" disabled selected>Pick one</option>
                                <option v-for="val of priorities" :key="val.id" :value="val.id">
                                    {{ val.name }}
                                </option>
                            </select>
                        </label>

                        <button class="mt-4 btn btn-secondary" @click="resetFilter">
                            Reset filters
                        </button>
                    </div>
                    <!-- Filter Dropdown : End -->
        ...
        <div v-else class="flex flex-col gap-4 bg-base-100 shadow-lg px-6 py-4 rounded-md">
            <div v-if="filteredTasks.length === 0">
                <div v-if="hasFilters" class="flex flex-col justify-center items-center gap-4 py-8">
                    <svg
                        class="w-12 h-12 text-base-content" data-slot="icon" fill="none" stroke-width="1.5"
                        stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                    >
                        <path
                            stroke-linecap="round" stroke-linejoin="round"
                ...
                <div v-else class="flex flex-col justify-center items-center gap-4 py-8">
                    <svg
                        class="opacity-25 w-12 h-12 text-base-content" data-slot="icon" fill="none" stroke-width="1.5"
                        stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                    >
            ...
            <TaskCard
                v-for="task in filteredTasks" v-else :key="task.id" :task="task" :categories="categories"
                :priorities="priorities" @status-changed="val => updateTaskStatus(task, val)" @task-deleted="loadTasks"
            />
        </div>
        ...
        <Modal :open="isCreating" @close="closeCreateModal">
            <div class="flex items-center gap-4 p-6">
                <svg
                    class="w-8 h-8 text-primary" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor"
            ...
            <form class="flex flex-col gap-4 px-6" @submit.prevent="createTask">
                <label class="form-control w-full">
                    <div class="label">
                        <span class="label-text">What is the task?</span>
                    </div>
                    <input
                        v-model="form.message" type="text" placeholder="Eg. Do the thing..."
                ...
                <div class="gap-4 grid grid-cols-1 md:grid-cols-2">
                    <label class="form-control">
                        ...
                        <select v-model="form.category_id" class="select-bordered select">
                            ...
                            <option v-for="val of categories" :key="val.id" :value="val.id">{{ val.name }}</option>
                        </select>
                    ...
                    <label class="form-control">
                        ...
                        <select v-model="form.priority_id" class="select-bordered select">
                            <option :value="null">Pick one</option>
                            <option v-for="val of priorities" :key="val.id" :value="val.id">{{ val.name }}</option>
                        </select>
            ...
            <div method="dialog" class="modal-backdrop">
                <div class="gap-4 grid grid-cols-2 mt-4 px-6 py-4 border-t border-base-300">
                    <button class="text-primary-content btn btn-primary" @click="createTask">
                        Create Task
                    </button>
                    <a class="bg-base-200 border border-base-300 btn" tabindex="0" @click="closeCreateModal">Close</a>
                </div>
            </div>
        </Modal>
    </div>
