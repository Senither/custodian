...
<template>
    <div
        class="border-primary/10 bg-primary/5 shadow px-4 py-3 border rounded-md transition-opacity" :class="{
    ...
    >
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <button class="bg-base-100 btn btn-square hover:btn-error" @click="isDeleting = true">
                    <svg
                        class="w-5 h-5 text-content" data-slot="icon" fill="none" stroke-width="1.5"
                ...
                <button class="bg-base-100 btn btn-square hover:btn-primary" @click="openEditModal">
                    <svg
                        class="w-5 h-5 text-content" data-slot="icon" fill="none" stroke-width="1.5"
                        stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                    >
        ...
        <Modal :open="isEditing" @close="closeEditModal">
            ...
            <form class="flex flex-col gap-4 px-6" @submit.prevent="saveTask">
            ...
            <div method="dialog" class="modal-backdrop">
                <div class="gap-4 grid grid-cols-2 mt-4 px-6 py-4 border-t border-base-300">
                    <button class="text-primary-content btn btn-primary" @click="saveTask">
                        Save Task
                    </button>
                    <a class="bg-base-200 border border-base-300 btn" tabindex="0" @click="isEditing = false">Close</a>
                </div>
            </div>
        </Modal>
        ...
        <Modal :open="isDeleting" @close="isDeleting = false">
            <div class="flex items-center gap-4 p-6">
                <svg
                    class="w-8 h-8 text-warning" data-slot="icon" fill="none" stroke-width="1.5" stroke="currentColor"
                    viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"
                >
                    <path
                        stroke-linecap="round" stroke-linejoin="round"
            ...
            <form class="modal-backdrop" @submit.prevent="deleteTask">
                <div class="gap-4 grid grid-cols-2 mt-4 px-6 py-4 border-t border-base-300">
                    <button type="submit" class="btn btn-warning">
                        Delete Task
                    </button>
                    <a class="bg-base-200 border border-base-300 btn" tabindex="0" @click="isDeleting = false">Close</a>
                </div>
            </form>
        </Modal>
    </div>
